    ...
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          ...
          <Pressable style={({ pressed }) => [s.oauthBtn, pressed && s.pressed]} onPress={handleGoogle}>
            <Image source={require('../../../assets/images/google_logo.png')} style={s.oauthIcon} />
            <Text style={s.oauthBtnText}>Continue with Google</Text>
            <Text style={s.chevron}>›</Text>
          </Pressable>

          <Pressable style={({ pressed }) => [s.oauthBtn, pressed && s.pressed]} onPress={handleGithub}>
            <Image source={require('../../../assets/images/github-logo.png')} style={s.oauthIcon} />
            <Text style={s.oauthBtnText}>Continue with GitHub</Text>
            <Text style={s.chevron}>›</Text>
          </Pressable>

          {Platform.OS === 'ios' && (
            <Pressable style={({ pressed }) => [s.oauthBtn, s.appleBtn, pressed && s.pressed]} onPress={handleApple}>
              <Image source={require('../../../assets/images/apple-logo.png')} style={s.oauthIcon} />
          ...
          {!showEmailForm ? (
            ...
            <View>
              ...
              >
                {loading ? <ActivityIndicator color="#1e4d2b" /> : <Text style={s.signInBtnText}>Sign In</Text>}
              </Pressable>
            </View>
          )}
...
const s = StyleSheet.create({
  ...
  oauthBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: WHITE_FAINT, borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  appleBtn: { backgroundColor: 'rgba(255,255,255,0.95)' },
  pressed: { opacity: 0.75 },
  oauthIcon: { width: 24, height: 24, marginRight: 12, resizeMode: 'contain' },
  oauthBtnText: { flex: 1, fontSize: 15, fontWeight: '600', color: WHITE },
  appleBtnText: { flex: 1, fontSize: 15, fontWeight: '600', color: '#000000' },
  chevron: { fontSize: 22, color: WHITE_DIM },
  chevronDark: { fontSize: 22, color: '#00000055' },
